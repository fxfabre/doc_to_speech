...
def erode(image: cv2.typing.MatLike, erosion_size=4) -> cv2.typing.MatLike:
    ...
    element = cv2.getStructuringElement(
        cv2.MORPH_RECT, (2 * erosion_size + 1, 2 * erosion_size + 1), (erosion_size, erosion_size)
    )
    return cv2.erode(image, element)


def remove_noise(gray_img: cv2.typing.MatLike, block_size=41, erosion_size=5) -> cv2.typing.MatLike:
    """
    Le seuillage local :
...
def sandbox_filtering():
    ...
    plt.subplot(152), plt.imshow(convolution_img, cmap='gray'), plt.title('2D convolution')
    plt.xticks([]), plt.yticks([])

    plt.subplot(153), plt.imshow(gauss_blur, cmap='gray'), plt.title('Gaussian Blurring')
    plt.xticks([]), plt.yticks([])

    plt.subplot(154), plt.imshow(median_blur, cmap='gray'), plt.title('Median Blurring')
    plt.xticks([]), plt.yticks([])

    plt.subplot(155), plt.imshow(bilateral, cmap='gray'), plt.title('Bilateral Filtering')
    plt.xticks([]), plt.yticks([])
